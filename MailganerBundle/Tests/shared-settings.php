...
try {
    ...
    $em->flush();
    $oldPlugin = $em->getRepository(Mautic\PluginBundle\Entity\Plugin::class)->findOneBy(['bundle' => 'MailganerBundle']);
    if ($oldPlugin) {
        $em->remove($oldPlugin);
        $em->flush();
    }
    ...
    $plugin->setBundle('MailganerBundle');
    $plugin->setName('Mailganer isolated registration');
    $plugin->setVersion('test');
    $subscriber = $c->get(MauticPlugin\MailganerBundle\EventSubscriber\SettingsLifecycleSubscriber::class);
    ...
    MauticPlugin\MailganerBundle\Model\SharedSettings::migrate($em, $plugin);
    $em->flush();
