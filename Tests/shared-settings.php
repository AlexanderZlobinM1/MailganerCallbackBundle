...
try {
    ...
    $em->flush();
    $oldPlugin = $em->getRepository(Mautic\PluginBundle\Entity\Plugin::class)->findOneBy(['bundle' => 'MailganerCallbackBundle']);
    if ($oldPlugin) {
        $em->remove($oldPlugin);
        $em->flush();
    }
    ...
    $plugin->setBundle('MailganerCallbackBundle');
    $plugin->setName('Mailganer isolated registration');
    $plugin->setVersion('test');
    $subscriber = $c->get(MauticPlugin\MailganerCallbackBundle\EventSubscriber\SettingsLifecycleSubscriber::class);
    ...
    MauticPlugin\MailganerCallbackBundle\Model\SharedSettings::migrate($em, $plugin);
    $em->flush();
