...
| Token-bucket fopen/flock/ftruncate/fwrite failures | The old callback plugins contain no token bucket. Full 1.4.0 adds a shared rate bucket with checked locking and atomic persistence; state failures stop new sends. Mailganer's receipt storage checks open/lock/write/sync/rename, uses atomic snapshots and fails before continuing on errors. |
